...
export class UsersHandler extends BaseRequestHandler {
    ...
    if (operationAuthorized) {
      ...
      if (parsedUrl) {
        if (parsedUrl.query.id) {
          const deleteResult 
            = await this.usersDBAccess.deleteUser(parsedUrl.query.id as string);
          if (deleteResult) {
  ...
  private async handleGet() {
    ...
    if (operationAuthorized) {
      ...
      if (parsedUrl) {
        if (parsedUrl.query.id) {
        const user 
          = await this.usersDBAccess.getUserById(parsedUrl.query.id as string);
          if (user) {
            ...
            const users 
              = await this.usersDBAccess.getUserByName(parsedUrl.query.name as string)
  ...
  private async operationAuthorized(operation: AccessRight)
    ...
    if (tokenId) {
      const tokenRights = await this.tokenValidator.
        validateToken(tokenId);
        console.log('Token rights: ' + tokenRights.accessRights[0]);
        console.log('Token state: ' + tokenRights.state.toString());
